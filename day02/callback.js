...
//         callback(num1+num2); //전달받은 함수를 사용하면서 매개변수에 결과를 전달한다.
//         return;
//     }
//     return num1 + num2;
// }
...
pay("감자", 720, 8, function(name, total){
    console.log(name + ": " + total + "원")
});


function stringCount(string, count, callback){
    if(callback){
        callback(string, count);
    }
}

stringCount("%", 5, function(string, count){
    var result="";
    for(i=0;i<count;i++){
        result +=string
    }
